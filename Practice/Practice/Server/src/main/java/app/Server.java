...
import server.EntityManagerFactoryUtil;
...
class ClientHandler implements Runnable {
	...
	public ClientHandler(Socket clientSocket) {
		...
		this.albumDAO = new AlbumDAO(this.entityManager);
	}
			...
			while (true) {
				...
				switch (choice) {
					case 1:
						...
						List<Albums> listAlbumByGenre = albumDAO.listAlbumByGenre(name);
						System.out.println("listAlbumByGenre=" + listAlbumByGenre);
						out.writeObject(listAlbumByGenre);
						out.flush();
						break;
					case 2:
						Map<String, Long> numberOfAlbumByGenre = albumDAO.getNumberOfAlbumsByGenre();
